        ...
        private int TextOutLine(string s, int x, int y, int c, int row)
        {
            ...
            for (int col = 0; col < s.Length;)
            {
                ...
                if (txt == "\t")
                {
                    ...
                    else if (px + textPixelWidth - m_ShareData.m_charWidthPixel * 4 > screenWidth)
                    {
                        break;
                    }
                    else
                    {
                        TextOutput(px, py, "^", charWidth);
                        px += charWidth;
                        dispText = "";
                        textPixelWidth = 0;
                ...
                else
                {
                    ...
                    charWidth = this.GetOneWordPixel(txt) + m_ShareData.m_nColumnSpace;
            ...
            if (dispText != "")
            {
                ...
                if ( s[col].ToString() == "\t")
                {
                    textPixelWidth = m_tabWidthPixel - ((px - x) % m_tabWidthPixel);
                    if (px + m_ShareData.m_charWidthPixel * 4 < 0) // ★文字間隔未考慮★
                    {
                        // 表示領域外（左側）
                        px += textPixelWidth;
                        dispText = string.Empty;
                    ...
                    else if (px + textPixelWidth - m_ShareData.m_charWidthPixel * 4 > screenWidth) // ★文字間隔未考慮★
                    {
                        // 表示領域外（右側）
                        invisible = true;
                        break;
                    ...
                    else
                    {
                        ...
                        CEWin32Api.SetTextColor(m_hDrawDC, CEConstants.SymbolFontColor);
                        CEWin32Api.SetBkColor(m_hDrawDC, CEConstants.BackColor);
                        TextOutput(px, py, "^", textPixelWidth);
                        px += textPixelWidth;
                        dispText = string.Empty;
                        textPixelWidth = 0;
                    }
                ...
                {
                    for (; col < s.Length && s[col].ToString() != "\t"; col++)
                    {
                        textPixelWidth += this.GetOneWordPixel(s[col].ToString()) + m_ShareData.m_nColumnSpace;
                        if (px + m_ShareData.m_charWidthPixel * 2 < 0) // 全角1文字分余裕を持たせる
                        {
                            // 表示領域外（左側）
                            px += textPixelWidth;
                            dispText = string.Empty;
                        ...
                        else if (px + textPixelWidth - m_ShareData.m_charWidthPixel * 2 > screenWidth) // 全角1文字分余裕を持たせる
                        {
                            // 表示領域外（右側）
                            invisible = true;
                            break;
            ...
            string dispText = string.Empty;
            int dispTextPixel = 0;
            int dFlag = 0; // 1:通常文字 / 2:タブ / 3:全角 / 4:コメント / 5:シンタックスハイライト
            ...
            for (int col = 0; col < s.Length; col++)
            {
                ...
                if (txt == "\t")
                {
                    ...
                    if (dFlag != 2)
                    {
                        TextOutput(px, py, dispText, dispTextPixel);
                        dispText = string.Empty;
                        dispTextPixel = 0;
                    }

                    dispText += txt;
                    dispTextPixel += textWidth;
                    dFlag = 2;
                ...
                else
                {
                    ...
                    if (dispText != "")
                    {
                        if (px + m_ShareData.m_charWidthPixel < 0) // 全角文字を考慮し、処理しようとしている文字の位置py（ピクセル）が、"-半角サイズ"の位置に来たら表示する
                        {
                            px += textWidth;
                            continue;
                        }
                        int w = this.GetOneWordPixel(dispText);
                        textWidth = w + (m_ShareData.m_nColumnSpace * (dispText.Length - 1));
                        CEWin32Api.SetTextColor(m_hDrawDC, c);
                        CEWin32Api.SetBkColor(m_hDrawDC, CEConstants.BackColor);
                        TextOutput(px, py, dispText, textWidth);
                        px += textWidth;
                    }
                }

                // 画面に表示されていない左側の文字は描画しないので飛ばす
                if (px + m_ShareData.m_charWidthPixel < 0) // 全角文字を考慮し、処理しようとしている文字の位置py（ピクセル）が、"-半角サイズ"の位置に来たら表示する
                {
                 //   px += textWidth;
                    continue;
                ...
                if (len != -1 && idx != -1)
                {
                    ...
                    {
                        // 検索
                        CEWin32Api.SetBkColor(m_hDrawDC, CEConstants.SearchBackColor);
                        CEWin32Api.SetTextColor(m_hDrawDC, CEConstants.SearchFontColor);
                    }
                }
                ...
                if (IsRngStr(row, col))
                {
                    ...
                    {
                        // 矩形選択
                        CEWin32Api.SetTextColor(m_hDrawDC, CEConstants.SelectRctFontColor);
                    }
                    else
                    {
                        // 通常選択
                        CEWin32Api.SetTextColor(m_hDrawDC, CEConstants.SelectFontColor);
                    }

                    if (m_searchType == SEARCH_TYPE)
                    {
                        // 検索
                        CEWin32Api.SetBkColor(m_hDrawDC, CEConstants.SearchMarkBackColor);
                        CEWin32Api.SetTextColor(m_hDrawDC, CEConstants.SearchMarkFontColor);
                    }
                    else
                    {
                        // 未検索
                        CEWin32Api.SetBkColor(m_hDrawDC, CEConstants.SelectBackColor);
                    }
                }
                ...
                // 文字描画
                //TextOutput(px, py, txt, textWidth);
                ...
                //CEWin32Api.ExtTextOut(m_hDrawDC, px + m_startColPos, py + m_startRowPos, (uint)flag, ref rcClip, txt, (uint)txt.Length, null);
