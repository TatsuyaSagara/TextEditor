...
namespace CETextBoxControl
{
    ...
    public class CEEditView : Control
    {
        ...
        /// 縦スクロールの移動量★★★
        /// </summary>
        private int m_scrollAmountMoveV;
        ...
        /// <param name="refreshFlag">差分描画有無（true:差分描画 / false 全描画）</param>
            ...
            if (nPos != 0)
            {
                ...
                if (refreshFlag)
                {
                    ...
                    unsafe
                    {
                        ...
                        CEWin32Api.GetClientRect(this.Handle, out rc);
                        if (offsetY > 0) // 上にスクロール
                        {
                            clipRect = new Rectangle(0, 0 + m_startRowPos,
                                        rc.right, rc.bottom - Math.Abs(offsetY));
                            CEWin32Api.RECT clip = new CEWin32Api.RECT(clipRect);
                            CEWin32Api.RECT rrr;
                            CEWin32Api.ScrollWindowEx(this.Handle, 0, offsetY, &clip, &clip, IntPtr.Zero, &rrr, CEWin32Api.SW_INVALIDATE);
                        }
                        else if (offsetY < 0) // 下にスクロール
                        {
                            clipRect = new Rectangle(0, Math.Abs(offsetY) + m_startRowPos,
                                         rc.right, rc.bottom - Math.Abs(offsetY));
                            CEWin32Api.RECT clip = new CEWin32Api.RECT(clipRect);
                            CEWin32Api.RECT rrr;
                    ...
                    CEWin32Api.SetScrollPos(Handle, false, 1/*this.FirstVisibleLine*/);
                    CEWin32Api.SetScrollPos(Handle, true, 0/*this.ScrollPosX*/);
                }
                else
                {
                    // 差分描画なし
        ...
        private void MoveScrollH(int nPos)
        {
            ...
            if (nPos != 0)
            {
                ...
                //CEWin32Api.SetScrollInfo(this.Handle, CEWin32Api.SB_HORZ, ref m_scrollInfoH, true);
#if false
                CEWin32Api.ScrollWindowEx(this.Handle, -nPos * (m_ShareData.m_charWidthPixel + m_ShareData.m_nColumnSpace), 0, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero, CEWin32Api.SW_ERASE | CEWin32Api.SW_INVALIDATE);
#else
                // 再描画
                CEWin32Api.InvalidateRect(this.Handle, IntPtr.Zero, true);
                //this.Refresh();
        ...
        /// ルーラー出力
        /// </summary>
        //static int viewRulerPos = -1;
        private void DispRuler()
        {
            //// 現在表示中のルーラ位置と文字表示位置が同じならルーラーの再描画はしない            
            //if (viewRulerPos == m_viewLeftColumn) return;
            //viewRulerPos = m_viewLeftColumn;
        ...
        private void GetCaretPositionV(/* int nVKey,*/ int nPos)
        {
            ...
            {
                m_scrollAmountMoveV = nPos;
                // 上または下移動
                // （キャレットの縦座標は変わらないので、m_caretPixel.Yは変更なし）
                MoveScrollV(nPos, line, true); // ★★★ 上下キーの時は、InvalidateRect()を使わずにScrollWindowExを使用する（高速化のため）
            }
        ...
        private void DrawTextList()
        {
            ...
            try
            {
                ...
                paintRect = new Rectangle(ps.rcPaint.left, ps.rcPaint.top, ps.rcPaint.right, ps.rcPaint.bottom - ps.rcPaint.top);
                ...
                IntPtr hDrawBmp = CEWin32Api.CreateCompatibleBitmap(hdc, rec.right, ps.rcPaint.bottom - ps.rcPaint.top);
...
#else
                // _DC : m_hDrawDC / _MemCD : m_hDrawDC
                IntPtr _MemBmp = CEWin32Api.CreateCompatibleBitmap(m_hDrawDC, paintRect.Width, paintRect.Height);
                Point _Offset = paintRect.Location;
                Size _MemDcSize = paintRect.Size;
                IntPtr _OrgMemBmp = CEWin32Api.SelectObject(m_hDrawDC, _MemBmp);
#endif
                // 背景色を白に設定
                IntPtr hDefBrush = CEWin32Api.SelectObject(m_hDrawDC, CEWin32Api.CreateSolidBrush(CEConstants.BackColor));
                //CEWin32Api.PatBlt(m_hDrawDC, 0, 0, paintRect.Width, paintRect.Height, CEWin32Api.TernaryRasterOperations.PATCOPY);
                ...
                // 描画（ビットマップ転送）
                //IntPtr hDC = CEWin32Api.BeginPaint(this.Handle, out ps);
                //CEWin32Api.BitBlt(hDC, ps.rcPaint.left, ps.rcPaint.top, ps.rcPaint.right, ps.rcPaint.bottom - ps.rcPaint.top, m_hDrawDC, 0, 0, CEWin32Api.TernaryRasterOperations.SRCCOPY);
...
#else
                // _DC : m_hDrawDC / _MemCD : m_hDrawDC
                CEWin32Api.BitBlt(hDC, _Offset.X, _Offset.Y, _MemDcSize.Width, _MemDcSize.Height, m_hDrawDC, 0, 0, CEWin32Api.TernaryRasterOperations.SRCCOPY);
                //RemoveClipRect();

                // dispose resources used in off-screen rendering
                CEWin32Api.SelectObject(m_hDrawDC, _OrgMemBmp);
                //CEWin32Api.DeleteDC(m_hDrawDC);
                //m_hDrawDC = IntPtr.Zero;
                CEWin32Api.DeleteObject(_MemBmp);
                _MemBmp = IntPtr.Zero;
            ...
            try
            {
                ...
                for (int index = 0; index < m_viewDispRow + 1; index++)
                {
                    ...
                    if (row < lineCntP)
                    {
                        ...
                        // 必要な部分以外は描画しない ★★★
                        if (m_scrollAmountMoveV < 0) // 下にスクロール
                        {
                            if (m_scrollAmountMoveV + index >= 0)
                            {
                                pLine++;
                                continue;
                            }
                        }
                        else if (m_scrollAmountMoveV > 0) // 上にスクロール
                        {
                            if ((m_scrollAmountMoveV > 0) && (m_viewDispRow - m_scrollAmountMoveV > index))
                            {
                                pLine++;
                                continue;
                            }
        ...
        private int GetOneWordPixel(string s)
        {
            ...
            CEWin32Api.GetTextExtentPoint32(m_hDrawDC, s, 1, out sz);
#else
            int[] extents = new int[s.Length];
            int drawableLength;
            sz = CEWin32Api.GetTextExtent(m_hDrawDC, s, 1, Int32.MaxValue, out drawableLength, out extents);
#endif
            return sz.Width;
        }
